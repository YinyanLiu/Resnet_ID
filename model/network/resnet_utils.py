...
def stack_blocks_dense(net,
  ...
  'block_name/unit_1', 'block_name/unit_2', etc. 创建命名空间

  Second, this function allows the user to explicitly control the ResNet
  output_stride, which is the ratio of the input to output spatial resolution.
  This is useful for dense prediction tasks such as semantic segmentation or
  object detection.
  其次，这个函数允许用户显式的控制输出的步长output_stride，也就是输入与输出之间的空间分辨率比例；
  ...
  current_stride = 1  # 随着stride变化每隔一个blocks倍增一次，它用来跟踪当前的stride
    ...
    with variable_scope.variable_scope(block.scope, 'block', [net]) as sc:
      for i, unit in enumerate(block.args):
        ...
        with variable_scope.variable_scope('unit_%d' % (i + 1), values=[net]):
          ...
          if output_stride is not None and current_stride == output_stride: # 二者相等，开始使用带孔卷积控制输出特征图大小
