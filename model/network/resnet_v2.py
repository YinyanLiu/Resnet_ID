...
def bottleneck(inputs,
               depth,
               depth_bottleneck, # 残差单元的前面两层的通道数
               stride,
               rate=1,
  ...
  with variable_scope.variable_scope(scope, 'bottleneck_v2', [inputs]) as sc:
    ...
    if depth == depth_in: # 通道数目，残差单元的输出层（最后一层）的通道数
    ...
    output = shortcut + residual  # shortcut + residual
...
def resnet_v2(inputs,
  ...
  在 imagenet 上训练分类网络通常使用[224, 224] 的输入，经过2^5=32的下采样后，得到 7*7 的特征图；

  如果是预测密集像素点的分类，那么建议使用 (32 * n + 1) 的输入尺寸，比如321*321，因为这样就会得到

  [(height - 1) / output_stride + 1, (width - 1) / output_stride + 1]的输出，并且得到的

  特征图严格对齐输入的图片，输入 225*225 得到 8*8 的特征图。
  For dense prediction tasks, the ResNet needs to run in fully-convolutional
  (FCN) mode and global_pool needs to be set to False. The ResNets in [1, 2] all
  have nominal stride equal to 32 and a good choice in FCN mode is to use
  output_stride=16 in order to increase the density of the computed features at
  small computational and memory overhead, cf. http://arxiv.org/abs/1606.00915.
  在预测密集像素点的分类的情况下，resnet 需要使用全卷积模式，并且不使用全局池化，

  建议使用 output_stride=16 来增加输出特征图密度而不过分增加计算量和内存使用。
  ...
  Args:
    ...
    global_pool: If True, we perform global average pooling before computing the
      ...
      是否使用全局池化，是的话为图片分任务，否的话为密集像素点的分类任务

    output_stride: If None, then the output will be computed at the nominal
      network stride. If output_stride is not None, it specifies the requested
      ratio of input to output spatial resolution. 输出的压缩比例
