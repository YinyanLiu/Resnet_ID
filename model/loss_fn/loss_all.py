...
class Loss(object):
    ...
    def cosineface_losses(self, embedding, labels, out_num, w_init=None, s=30., m=0.4):
        '''
        :param embedding: the input embedding vectors
        :param labels:  the input labels, the shape should be eg: (batch_size, 1)
        ...
        with tf.variable_scope('cosineface_loss'):
            ...
            output = tf.add(s * tf.multiply(cos_t, inv_mask), s * tf.multiply(cos_t_m, mask),
                            name='cosineface_loss_output')
        return output

    def combine_loss_val(self, embedding, labels, w_init, out_num, margin_a, margin_m, margin_b, s):
        '''
        ...
        accuracy_s = tf.reduce_mean(
            tf.cast(tf.equal(tf.cast(predict_cls_s, tf.int64), tf.cast(labels, tf.int64)), 'float'))
